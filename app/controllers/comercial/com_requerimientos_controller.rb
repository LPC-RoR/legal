class Comercial::ComRequerimientosController < ApplicationController
  ...
  def create
    ...
    # Marcar como verificado automáticamente (confiamos en el anti-bot)
    @objeto.email_ok = @objeto.email

    respond_to do |format|
      if @objeto.save

        # NUEVO: Notificar al equipo comercial
        Contexts::Platform::ContactoComercialMailer
          .nueva_solicitud(@objeto.id)
          .deliver_later


        format.html { redirect_to root_path, notice: "Gracias por contactarnos. Nuestro equipo comercial se comunicará contigo en las próximas 24 horas." }
