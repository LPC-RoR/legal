class Karin::KrnDenunciantesController < ApplicationController
  ...
  before_action :set_krn_denunciante, only: %i[ show edit update destroy cargar_pdf swtch rlzd prsnt set_fld ]
  ...
  def create
    ...
    respond_to do |format|
      if @objeto.save
        ...
        format.html { redirect_to shw_dnnc_tab_indx(@objeto.krn_denuncia, 0), notice: "Denunciante fue exitosamente creado." }
  ...
  def verify
    ...
    redirect_to dshw_dnnc_tab_indx(@objeto.krn_denuncia, 0), alert: 'Token inválido'
  end
  ...
  def update
    respond_to do |format|
      if @objeto.update(krn_denunciante_params)
        format.html { redirect_to shw_dnnc_tab_indx(@objeto.krn_denuncia, 0), notice: "Denunciante fue exitosamente actualizado." }
  ...
  def destroy
    ...
    respond_to do |format|
      format.html { redirect_to shw_dnnc_tab_indx(@objeto.krn_denuncia, 0), notice: "Denunciante fue exitosamente eliminado." }
  ...
  private
    ...
    def krn_denunciante_params
      params.require(:krn_denunciante).permit(:krn_denuncia_id, :krn_empresa_externa_id, :rut, :nombre, :cargo, :lugar_trabajo, :email, 
        :email_ok, :articulo_4_1, :articulo_516, :direccion_notificacion, :representante, :empleado_externo, :telefono, :dclrcn_intrrmpd)
    end
end
