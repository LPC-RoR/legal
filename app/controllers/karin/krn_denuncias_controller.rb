class Karin::KrnDenunciasController < ApplicationController
  ...
  before_action :set_krn_denuncia, only: %i[ show edit update destroy swtch niler rlzd prsnt pdf_combinado pdf_designacion pdf_notificaciones annmzr set_fld prg krn_pdf_rprt ]
  ...
  def show
    ...
    when 3
      ...
      @rcrss_infrm  = @objeto.act_archivos.where(act_archivo: ['dsgncn_invstgdr', 'ntfccns'])
  ...
  private
    ...
    def krn_denuncia_params
      params.require(:krn_denuncia).permit(:ownr_type, :ownr_id, :identificador, :fecha_hora, :receptor_denuncia, :motivo_denuncia, :krn_empresa_externa_id, :krn_investigador_id, :fecha_hora_dt, :presentado_por, :via_declaracion, :tipo_declaracion, :representante, :auditoria, :fecha_trmtcn)
    end
end